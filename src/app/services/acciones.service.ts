...
export class AccionesService {
  ...
  async emitirNuevoPedido(
    ...
    if (cliente !== null) {
      this.wsService.emit('emitir-nuevo-pedido', {
        nombres: cliente.Nombres + ' ' + cliente.Apellidos,
        plataforma: plataforma,
        idalmacen: idAlmacen,
        iddbempresa: iddbEmpresa,
        tienesistema: tieneSistema,
        idregion: idRegion,
      });
    }
  }
  ...
  async emitirCanceladoPedido(idPedido: any, idEmpresa: any) {
    let cliente = await this.storage.get('cliente');
    if (cliente !== null) {
      this.wsService.emit('emitir-cancelado-pedido', {
