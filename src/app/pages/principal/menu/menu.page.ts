    ...
    await this.storage.get('cliente').then(async (val) => {
      ...
      try {
        ...
        if (respuestaNotificacionesNum.data.success == true) {
          this.herramientasService.setNotificaciones(
            respuestaNotificacionesNum.data.data
          );
        } else {
...
/* 
       this.callNotifications =  setInterval(async () => {
        ...
        if (respuestaNotificacionesNum.data.anulado == 1) {
          ...
          this.navCtrl
            .navigateRoot('/login', { animationDirection: 'back' })
            .then(() => {
              setTimeout(() => {
          ...
          const alert = await this.alertController.create({
            ...
            backdropDismiss: false,
            message:
              'Su cuenta ha sido bloqueda pueder ser por causas como pedidos falsos, envio masivo de mensajes, etc.  Si es un error comunicate con nosotros vía whastapp (+51 997 578 199)  .',
            mode: 'ios',
            buttons: [
              {
                cssClass: 'primarybtn',
    ...
    if (this.departamentoSelect == null) {
      const alert = await this.alertController.create({
        ...
        backdropDismiss: false,
        message:
          'Para una mejor experencia porfavor  seleccione su ciudad actual  ',
        mode: 'ios',
        buttons: [
          {
            cssClass: 'primarybtn',
  ...
  async onIrCategoria(
    ...
    if (departamento == null) {
      ...
      try {
        let resp = await this.dataService.getEmpresaDirecta(
          idCategoria,
          departamento.DepartamentoUbicacion
        );
        if (resp.data.success) {
          this.navegacionService.setEmpresa(resp.data.data);
          this.navCtrl.navigateRoot('/principal/empresa', {
      ...
      try {
        ...
        if (respuestaEmpresas.data.success == true) {
          ...
          this.subCategoria = this.empresas
            .map((a: any) => ({
              name: a.SubCategoria,
              ImagenUrlSubCate: a.ImagenUrlSubCate,
            })) // Mapea con un objeto que contenga name
            .filter(
              (v: any, i: any, a: any) =>
        ...
        this.mostrarMensajeBottom(
          'Error: No se cargaron correctamente las empresas',
          2000,
          'danger'
        );
      }
  ...
  filtro(SubCategoria: string) {
    return this.empresas.filter(
      (empresa: any) => empresa.SubCategoria == SubCategoria
    );
  }
  ...
  buscarProducto(e: any) {
    if (e.detail.value.trim() != '') {
      this.showListEmpresas = this.empresas.filter((item: any) =>
        item.RazonSocial.toLowerCase().includes(
          e.detail.value.toLowerCase().trim()
        )
      );
    }
  }
  ...
  ngOnDestroy(): void {
   // clearInterval(this.callNotifications)
  }
}
